#include <iostream>
#include <conio.h>					//	�Hola profe! Para moverse por medio del codigo con los comentarios
#include <windows.h>				//	utiles respecto a este, utilize la herramienta de busqueda de palabras
#include <cstdlib>					//	y teclee "Indi" y el numero de la derecha indica que lugar est� en el codigo.
#include <math.h>
#include <fstream>
#include <thread>
#include <stdlib.h>
...
// handle is an integrer index to a pointer to a resourse in kernel space
void gotoxy(int,int);//Funcion declarada despues del main

void Change(int color){ // Indi 01 "Aqu� se encuentra change, cambia elcolor del fondo y letra que se imrime al momento"
    HANDLE hConsole;	//						Pertenece a C, pero funciono en el nuestro, por eso aqu� esta. Usa HANDLE. Fin.
...
void renglon (int request,int nively) {
    ...
    if (request==2){ //               IMPRIME ELINTERMEDIO DEL TABLERO
        ...
        cout<<(char)180;
        Change(253);
        if (nively==1) {//              TODOS LOS "NIVELY" SON PARA EL MENU A LA DERECHA nivel del renglon
            cout << "   "<<(char)201;
        ...
        if (nively==3) {
                cout << "     (C) CARGAR la partida.   (M) RESOLVER sudoku";
        }
        if (nively==4) {
            cout << "   "<<(char)201;
...
void celda (int num, int point, int cc, int colorFONT){ // Indi 02
    /* COLOR FONT
    0 = Nada
    1 = Letra VERDE
    2 = Letra ROJA
    3 =
    4 Aqu� se imprime el contenido de la celda, que ser�a el cuadrado donde estas escribiendo (o no) el numero
    5
    */
    if (colorFONT==1)
        Change(242);
...
bool EncontrarCeldaVacia(int n[9][9], int &xSol, int &ySol){ 	// Indi 03.- Funci�n que recorre el arreglo para encontrar celdas vac�as
...
bool ComprobarFila(int n[9][9], int xSol, int numSol) {				//Comprueba si hay un n�mero igual al que se est� comprobando en la misma fila
...
bool ComprobarColumna(int n[9][9], int ySol, int numSol) {		//Comprueba si hay un n�mero igual al que se est� comprobando en la misma columna
...
bool Comprobar3x3(int n[9][9], int boxStartX, int boxStartY, int numSol){			//Comprueba si hay un n�mero igual al que se est� comprobando en la misma cuadr�cula de 3x3
...
bool Legal(int n[9][9], int xSol, int ySol, int numSol){			// Si el numero que se est� comprobando cumple con las 3 condiciones (no fila, columna ni cuadr�cula), es "legal"
																															// usar ese numero en esa casilla
    return !ComprobarFila(n, xSol, numSol) &&
           !ComprobarColumna(n, ySol, numSol) &&
           !Comprobar3x3(n, xSol - xSol%3 , ySol - ySol%3, numSol);
}

bool SolveSudoku(int n[9][9],int imprimir){				// Indi 04 Funcion que recibe la partida en la posicion actual y la devuelve e imprime resuelta

    int xSol, ySol; 															// Indican la posici�n en el eje X y Y de la celda en cuesti�n

    if(!EncontrarCeldaVacia(n, xSol, ySol))				// Si no se encuentra una celda vac�a, el sudoku est� resuelto
       return true;
    for(int numSol = 1; numSol <= 9; numSol++){
        if(Legal(n, xSol, ySol, numSol))					// Recorre los numeros del 1 al 9 para encontrar si hay uno legal,
        {																					// si lo hay, le asigna ese valor; si no lo hay, regresa al valor anterior (recursividad)
            n[xSol][ySol] = numSol;
            ...
            if(SolveSudoku(n,imprimir))						// Aqu� es donde se aplica la recursividad, volviendo a ejecutar la funci�n hasta que la confici�n se cumpla
                return true;
...
// Indi 05 Aqu�, es solo que imprima el sudoku MUY chido. El resto son parte de la cuadricula y renglones del menu de la derecha
...
// Indi 06 funcion hecha por terminos de eficiencia de excritura de un menor numero de lineas de codigo al evitarnos escribir lo mismo en dos case del switch
void SudokuMaker(bool choice){
    ...
    int r[9][9];
// Indi 07 El choice es sise eligio pla plantilla vacia o no. Esto es el nucleo del sudoku jugable. Lee el archivo y...
  // establece que se puede modificar o no al momento de jugar en la cuadricula de 9 X 9
    try{
  	    if(choice== true){
  	      Lista.open("plantilla1.txt");
	        if(!Lista.is_open())throw bad_exception();
        ...
        system("cls");
      cerr << "\n Error: El archivo 'plantilla1.txt' no esta en la ubicacion de este programa." << endl;
    ...
    while (0==0) { // Indi 08   ESTE WHILE ES EL MANEJO DE LA TABLA DEL SUDOKU Y SUS VALORES
        ...
        gotoxy(gotx+1,goty); //ESTO APUNTA A LA SELECCI�N DE LA CELDA ACTUAL
			// Indi 09 USAMOS _gets igual que HANDLE, y nos sirvio tambien. por eso sigu� aqu�. El resto de ifs son los comandos de teclas
        keys=_getch();
        advertencia=0;
...
int main()
{
  thread t;
	...
	int menu_item=0, x=7;// Indi 10		menu_item es para la seleccion que haga el usuario en
    ...
    gotoxy(18,5); cout << "         Dr. Raymundo Cornejo Garcia          "<<endl;
    ...
    getch();
    system("cls");// Indi 11		funcion para hacer una limpia de pantalla
...
void gotoxy(int x, int y) // Indi 12 Sepa por que bajaron el gotoxy, pero bueno, se encarga de mover el puntero de impresion de la consola.
  // Recive un par de coordenadas y MAGIA. Junto con ocultar cursor y Celda, tenemos una seleccion personalizada.
{
	CursorPosition.X = x;
	CursorPosition.Y = y;
